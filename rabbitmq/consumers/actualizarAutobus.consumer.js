...
class ActualizarAutobusConsumer {
    ...
    connectToQueue(id) {
        ...
        //         globalConnection.channel = channel;
                ...
                global.connection.channel.assertQueue(queueName, { exchange: true, durable: true }, (err2, q) => {
                    ...
                    global.connection.channel.bindQueue(q.queue, exchange.nombre, exchange.routingKey.actualizarCamion);

                    global.connection.channel.consume(q.queue, (msg) => {
                        if (msg.content) {
                            console.log("ActualizarAutobus.consumer [x]% s", msg.content.toString());
...
module.exports = ActualizarAutobusConsumer;